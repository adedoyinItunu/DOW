...
def _diurnal_base(rng, amp=30.0):
    hours = np.arange(HOURS)[:, None]
    days = np.arange(DAYS)[None, :]
    daily = 0.15 + np.exp(-((hours - 13.0) ** 2) / (2 * 3.5 ** 2))
    weekly = np.where(np.isin(days % 7, [5, 6]), 0.7, 1.0)
    return rng.poisson(amp * daily * weekly).astype("float32")


def _envelope():
    hours = np.arange(HOURS)[:, None]
    return 0.15 + np.exp(-((hours - 13.0) ** 2) / (2 * 3.5 ** 2))


def _gen_normal(rng, intensity=0):
    return _diurnal_base(rng)


def _gen_linear(rng, intensity):
    g = _diurnal_base(rng)
    days = np.arange(DAYS)[None, :]
    if intensity == 1:                       # leech: subtle, hidden in the daily shape
        slope = rng.uniform(8, 16)
        extra = _envelope() * (days / (DAYS - 1)) * slope
...
def _gen_geometric(rng, intensity):
    g = _diurnal_base(rng)
    days = np.arange(DAYS)[None, :]
    base = rng.uniform(0.3, 0.7) if intensity == 1 else rng.uniform(1.2, 2.5)
    surge = np.clip(base * (1.18 ** days), 0, 130)
    return g + rng.poisson(np.broadcast_to(surge, g.shape)).astype("float32")


def _gen_random(rng, intensity):
    g = _diurnal_base(rng)
    speckle = np.zeros_like(g)
    if intensity == 1:
        n, lo, hi = rng.integers(30, 60), 20, 45
    else:
        n, lo, hi = rng.integers(80, 140), 50, 100
...
def generate_dataset(per_class=300, seed=42):
    """
...
def save_preview(path="data_preview.png", seed=1):
    ...
    for ax, (title, c, lvl) in zip(axes, rows):
        ax.imshow(_GEN[c](rng, lvl), origin="lower", aspect="auto", cmap="magma")
...
def main():
    ...
    if args.preview:
        save_preview(); return
    X, y, inten = generate_dataset(args.per_class, args.seed)
