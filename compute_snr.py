...
The reviewer's objection: the leech is constructed by scaling injected load
...
Compare the scale at which d' crosses 1 against the scale at which your
...
exploit. It is the relevant quantity, not the per-cell SNR.
  ...
  * If d' stays WELL ABOVE 1 at the scales where your detector collapses
    ...
    the models tested -- your current claim stands.
  ...
  * If d' falls to around 1 at those same scales, the signal has dropped
    ...
    INFORMATIONAL -- a property of the detection problem. Your finding is
    still real and better grounded, but Sections 4.6 and 5.3 must be
    reworded: it is not the heat-map representation that imposes the limit.
